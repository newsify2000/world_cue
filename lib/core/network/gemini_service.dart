...
class NewsSummarizerService {
  ...
  String _getLongCacheKey(String title) => 'long_summary_$title';

  /// Generates a 60-word summary of a news article using the Gemini API.
  Future<String> getNewsShortSummary({
...
''';
  ...
  /// Generates a detailed summary of a news article using the Gemini API.
  Future<String> getNewsDetailedSummary({
    required String sourceLink,
    required String newsText,
    required String title,
  }) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    final cacheKey = _getLongCacheKey(title);
    ...
    if (isPresent == null) {
      ...
      final String prompt =
          '''
Please write a detailed news summary of the provided article in 3 to 5 paragraphs. The summary must include the main announcement, the context/significance of the achievement, and a brief overview of the method or initiative used to reach the outcome. Maintain a formal, journalistic tone.
It is for my application, so the paragraph should look like an app is returning a result. Do not include any introductory phrases like 'The article summarizes...' or questions.
